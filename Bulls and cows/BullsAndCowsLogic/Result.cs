...
namespace BullsAndCowsLogic
{
    public class Result
    {
        private List<Utils.eResult> m_ResultFeedback = new List<Utils.eResult>();
        ...
        public Result(Pins i_PlayerGuess, Pins i_ComputerSequence)
        {
            ...
            for (int i = 0; i < this.m_PlayerGuess.Sequence.Count; i++)
            {
                ...
                {
                    this.m_ResultFeedback.Add(Utils.eResult.V);
                    numOfAccurateGuess++;
                }
                else if (i_ComputerSequence.Sequence.Contains(i_PlayerGuess.Sequence[i]))
                {
                    this.m_ResultFeedback.Add(Utils.eResult.X);
                }
            }

            this.m_ResultFeedback.Sort();
